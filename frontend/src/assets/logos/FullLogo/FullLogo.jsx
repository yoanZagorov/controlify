...
export default function FullLogo({ color = COLORS.THEME.NAVY, className }) {
  ...
  useLayoutEffect(() => {
    if (svgRef?.current && textRef?.current) {
      document.fonts.ready.then(() => {
        // have to await the fonts to ensure proper calculations (this causes a little bit of flickering but calculations are sure to be correct)
  ...
  const letterEls = LOGO_LABEL.split('').map((letter, i) => (
    <tspan key={`tspan-${i}`} dx={i === 0 ? 0 : textGapWidth}>
      {letter}
    </tspan>
  ))
