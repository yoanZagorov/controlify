...
export default function TransactionItem({
  action,
  isExpanded,
  transaction: { amount, currency, type, category, wallet, date },
  display,
}) {
  const { transactionData, defaultTransactionData, resetTransactionData } =
    useTransaction()
  ...
  const hasTransactionDataChanged = useMemo(
    () =>
      ...
      JSON.stringify(defaultTransactionData),
    [transactionData],
  )
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`flex items-center ${isExpanded ? 'gap-4' : 'gap-2.5'}`}
        >
          <div
          ...
          >
            <SvgIcon
              ...
              className="size-1/2 fill-gray-light"
            />
          </div>
            ...
            {display.wallet && (
              ...
              >
                <SvgIcon
                  ...
                  className="size-3 min-w-3 fill-current"
                />
                <span className="text-left text-xs lm:text-sm">
                  {formattedWalletName}
                </span>
              </div>
            )}
        ...
        <Amount
          ...
          className={cn(
            ...
            isSpaceLimited ? 'text-sm' : isExpanded ? 'text-lg' : 'text-base',
          )}
        />
      </button>
    </TransactionContainer>
