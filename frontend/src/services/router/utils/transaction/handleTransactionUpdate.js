...
export default async function handleTransactionUpdate(userId, formData) {
  ...
  try {
    ...
    const oldTransactionData = await getEntity(
      transactionDocRef,
      transactionId,
      'transaction',
    )
    ...
    if (hasDataChanged.amount) {
      ...
      validateAmount(
        amount,
        VALIDATION_RULES.TRANSACTION.AMOUNT.MIN_AMOUNT,
        VALIDATION_RULES.TRANSACTION.AMOUNT.MAX_AMOUNT,
        'transaction amount',
      )

      // Update wallet balance if amount has changed
      const walletDocRef = doc(db, `users/${userId}/wallets/${walletId}`)
      const { balance: oldWalletBalance, currency: walletCurrency } =
        await getEntity(walletDocRef, walletId, 'wallet')
      ...
      if (oldTransactionData.currency !== walletCurrency) {
        amountDifference = await getConvertedAmount(
    ...
    const transactionUpdatePayload = getDataToChange(hasDataChanged, {
      amount,
      category: categoryTransactionPayload,
      date,
    })
    ...
    return handleActionError(
      error,
      "Couldn't update your transaction. Please try again",
    )
  }
}
