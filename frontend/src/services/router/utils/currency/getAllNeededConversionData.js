...
export default async function getAllNeededConversionData({
  userId = null,
  walletId = null,
  periodInfo = {},
  neededData,
  ...
  if (
    neededData.includes('periodTransactionsByWallet') &&
    !conversionData.periodTransactionsByWallet
  ) {
    ...
    conversionData.periodTransactionsByWallet = await getPeriodTransactions({
      userId,
      providedWallets,
      periodInfo,
      byWallet: true,
  ...
  if (
    neededData.includes('nonBaseCurrenciesRates') &&
    !conversionData.nonBaseCurrenciesRates
  ) {
    conversionData.nonBaseCurrenciesRates = await getNonBaseCurrenciesRates({
      ...
      ...(neededData.includes('userCurrency')
        ? { userCurrency: conversionData.userCurrency }
        : {}),
    })
  }
