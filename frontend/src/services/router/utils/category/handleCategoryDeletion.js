...
export default async function handleCategoryDeletion(userId, formData) {
  ...
  try {
    ...
    if (
      type === 'expense' &&
      sameTypeCategories.length === CATEGORY.MIN_AMOUNT.EXPENSE
    ) {
      throw new ValidationError(
        `Your account should have a minimum of ${CATEGORY.MIN_AMOUNT.EXPENSE} expense categories`,
      )
    }
    if (
      type === 'income' &&
      sameTypeCategories.length === CATEGORY.MIN_AMOUNT.INCOME
    ) {
      throw new ValidationError(
        `Your account should have a minimum of ${CATEGORY.MIN_AMOUNT.INCOME} income categories`,
      )
    }
    ...
    activeWallets.forEach((wallet) => {
      ...
      batch.update(walletDocRef, {
        [`categoriesVisibility.${id}`]: deleteField(),
      })
    })
    ...
    return handleActionError(
      error,
      "Couldn't delete your category. Please try again",
    )
  }
}
