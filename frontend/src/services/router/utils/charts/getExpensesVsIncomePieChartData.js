...
export default async function getExpensesVsIncomePieChartData(
  periodTransactions,
  ...
  const expenseTransactions = {
    name: 'expense',
    fill: COLORS.THEME.RED.DARK,
    amount: 0,
  }
  const incomeTransactions = {
    name: 'income',
    fill: COLORS.THEME.GREEN.DARK,
    amount: 0,
  }
  ...
  periodTransactions.forEach((transaction) => {
    ...
    if (type === 'expense') {
      expenseTransactions.amount = performDecimalCalculation(
        expenseTransactions.amount,
        convertedAmount,
        '+',
      )
    } else {
      incomeTransactions.amount = performDecimalCalculation(
