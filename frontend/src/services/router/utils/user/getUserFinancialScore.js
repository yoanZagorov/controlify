...
export default async function getUserFinancialScore(
  ...
  if (!isArrayTruthy(periodTransactions))
    return FINANCIAL_SCORE_AMOUNTS.BREAK_EVEN
  ...
  for (const transaction of periodTransactions) {
    ...
    if (type === 'income') {
      incomeAmount = performDecimalCalculation(
        incomeAmount,
        convertedAmount,
        '+',
        4,
      ...
      expenseAmount = performDecimalCalculation(
        expenseAmount,
        convertedAmount,
        '+',
        4,
  ...
  const financialScore = Math.trunc(
    Math.max(
      FINANCIAL_SCORE_AMOUNTS.MIN,
      Math.min(
        FINANCIAL_SCORE_AMOUNTS.MAX,
        FINANCIAL_SCORE_AMOUNTS.BREAK_EVEN +
          FINANCIAL_SCORE_AMOUNTS.BREAK_EVEN *
