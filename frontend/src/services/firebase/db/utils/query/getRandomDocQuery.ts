...
export default async function getRandomDocQuery(
  collectionRef: CollectionReference,
): Promise<[QueryConstraint[], QueryConstraint[]]> {
  const randomId = doc(collectionRef).id

  // Using arrays since that is how queries in the application are implemented
  const randomDocQuery: QueryConstraint[] = [
  ...
  const fallback: QueryConstraint[] = [limit(1)]
