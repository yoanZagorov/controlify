...
export default async function updateAuthEmail(email) {
  ...
  try {
    ...
    storeRedirectData("Please check your inbox and verify your new email address to finalize the update", "notification", ROUTES.SETTINGS);
    await signOut(auth);
    ...
    if (error.code === "auth/requires-recent-login") {
      ...
      storeRedirectData("This change requires a recent login. Please log in with your old email address and try again.", "notification", ROUTES.SETTINGS);
      await signOut(auth);
      return redirect(ROUTES.LOGIN);
    }

    throw new StatusCodeError("Unable to update your email address. Please try again", { cause: error });
  }
}