...
export default function NavItem({
  ...
  const getClasses = (isLink = false, isActive = null) =>
    cn(
      ...
      isLink &&
        (isActive
          ? `opacity-100 font-bold ${variantClasses.type[typeVariant].active}`
          : `opacity-80 hover:opacity-100 ${variantClasses.type[typeVariant].inactive}`),
    )

  const classes = {
    logoutBtn: getClasses(),
    navItem: ({ isActive }) => getClasses(true, isActive),
    icon: cn('fill-current', variantClasses.type[typeVariant].icon),
  }
