...
export default function SpendingSection() {
  ...
  const hasSufficientData = {
    expensesByCategory: chartData.expensesByCategory.find(
      (entry) => entry.amount > 0,
    )
      ? true
      : false,
    expensesVsIncome: chartData.expensesVsIncome.find(
      (entry) => entry.amount > 0,
    )
      ? true
      : false,
  ...
  const elements = [
    <ChartWrapper
      ...
      iconName="categories"
      title="by category"
      hasSufficientData={hasSufficientData.expensesByCategory}
    >
    ...
    <ChartWrapper
      ...
      hasSufficientData={hasSufficientData.expensesVsIncome}
    >
      <ExpensesVsIncomeVerticalBarChart
        currency={currency}
  ...
  return (
    ...
    >
      {isSingleColLayout ? (
        ...
        <div className="flex gap-16">{elements}</div>
      )}
    </Section>
  )
}
