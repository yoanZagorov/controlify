...
export default function validateProfilePic(profilePic) {
  ...
  if (!name) {
    throw new ValidationError(
      'File name should not be empty. Please submit a file',
    )
  }
  ...
  if (!VALID_FILE_TYPES.includes(type)) {
    throw new ValidationError(
      'Unsupported file type. Please try with another one',
    )
  }
  ...
  if (size > FIVE_MB) {
    throw new ValidationError(
      'File size should not exceed the maximum limit of 5MB. Please try with a smaller image',
    )
  }
}
